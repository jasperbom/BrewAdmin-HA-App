...
//     Merch die de brouwerij niet zelf op voorraad heeft staat in de
//     merch-lijst (`utils/merch`): die regels zijn geen gok maar een
//     expliciete keuze van de gebruiker en komen dus zonder
//     "onbekend"-waarschuwing binnen.
...
import { MerchArtikel, isMerch } from './merch'
...
export interface WcRefs {
  ...
  /** Merch-artikelen: wel verkocht, maar niet uit eigen voorraad geleverd. */
  merch?: MerchArtikel[]
}
...
export interface WcOrderRegel {
  ...
  wc_onbekend?: boolean
  /** Bekend merch-artikel: bewust geen picking, geen waarschuwing. */
  merch?: boolean
}
...
export function mapWcOrderRegels(order: any, refs: WcRefs = {}): WcOrderRegel[] {
  ...
  for (const item of (order?.line_items || [])) {
    ...
    // Expliciet als merch gemarkeerd? Dan niet meer naar een eigen artikel
    // zoeken: de gebruiker heeft al gezegd dat dit niet uit de eigen voorraad
    // geleverd wordt.
    const merchRegel = isMerch(sku, naam, refs.merch)
    const match = merchRegel ? null : vindWcArtikel(sku, naam, refs)
