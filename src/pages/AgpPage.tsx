...
function AgpPage({bat, av, uit, acc, setAcc, locaties, setLocaties, verplaatsingen, setVerplaatsingen, afboekingen, accijnsInst, log, setLog, auditLog, setAuditLog}: any) {
  ...
  const saveVerplaats = () => {
    ...
    // Voorraad die de AGP heeft verlaten kan niet meer terug — accijns is dan
    // al verschuldigd en mag niet 'gerestaureerd' worden.
    if (naar.is_agp) { alert(t('agp_err_naar_agp_verboden')); return; }
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select value={vplModal.naar_locatie_id} onChange={e=>setVplModal((f: any)=>({...f, naar_locatie_id: Number(e.target.value)}))} className="t-input w-full px-2.5 py-1.5 rounded text-sm bg-white border border-gray-200">
                        ...
                        {/* AGP is uitgesloten als bestemming: voorraad die uit AGP is gehaald kan niet terug */}
                  ...
                  {(() => {
                    ...
                    if (van && !van.is_agp && naar?.is_agp) {
                      // Onbereikbaar via de UI (filter sluit AGP uit), maar voor het geval een
                      // legacy state of programmatic flow toch hier komt: blokkeer-melding.
                      return <div className="bg-red-50 border border-red-200 rounded p-3 text-xs text-red-800">{t('agp_err_naar_agp_verboden')}</div>;
                    }
