...
function DashboardPage({ing, lots, bat, bi, uit, acc, setPage, tanks, gistMetingen=[], haInst, haTankTemps={}, setNavBatchId}: any) {
      ...
      {tanks && tanks.length>0 && (
        <div className="flex flex-wrap justify-center gap-4 mb-6">
          {tanks.map((tk: any)=>{
            ...
            const handleTankClick = anyBatch ? () => { setNavBatchId(anyBatch.id); setPage('batches') } : undefined;
            return (
              <div key={tk.id} onClick={handleTankClick}
                className={`bg-white rounded-xl shadow-sm border p-4 w-64 flex-shrink-0 ${anyBatch?'t-border cursor-pointer hover:shadow-md':'border-gray-200'}`}>
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {haTankTemps[tk.id] != null && (
                        <div className="text-sm font-bold text-gray-700 mt-1">{Number(haTankTemps[tk.id]).toFixed(2)}°C</div>
                      )}
                    </div>
                  </div>
                </div>
