    ...
    <div className="min-h-screen" style={{backgroundColor:'var(--t-bg)'}}>
      <nav className="text-white sticky top-0 z-40 shadow-lg border-b" style={navStyle}>
        <div className="max-w-7xl mx-auto px-4 flex items-center h-14 gap-2 overflow-x-auto">
          {logo
            ? <img src={logo} alt="logo" onClick={()=>setPage('dashboard')}
                style={{height:'32px',width:'auto',maxWidth:'80px',objectFit:'contain',cursor:'pointer',flexShrink:0}} />
            : <button onClick={()=>setPage('dashboard')} className="flex-shrink-0 opacity-90 hover:opacity-100" aria-label="Home">
                <svg width="30" height="30" viewBox="0 0 32 32" fill="currentColor" className="text-white">
                  {/* fermentation tank */}
                  <path d="M10 4 Q10 2 12 2 L20 2 Q22 2 22 4 L22 6 Q22 8 20 8 L12 8 Q10 8 10 6 Z"/>
                  <path d="M8 8 L24 8 L24 26 Q24 29 21 29 L11 29 Q8 29 8 26 Z"/>
                  <path d="M24 14 L28 14 Q30 14 30 16 Q30 18 28 18 L24 18"/>
                  <circle cx="14" cy="19" r="1.5" opacity="0.6"/>
                  <circle cx="19" cy="16" r="1.5" opacity="0.6"/>
                  <circle cx="16" cy="22" r="1.2" opacity="0.5"/>
                </svg>
              </button>}
