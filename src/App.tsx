...
function App() {
  ...
  // Merch die via de webshop verkocht wordt maar niet uit eigen voorraad komt
  // (dropshipping) — zie utils/dropship.ts.
  const [dropshipArtikelen, setDropshipArtikelen] = useStore('dropship_artikelen', []);
  ...
  const doExport = () => {
    excelExport({
      ...
      artikelen,
      dropship_artikelen: dropshipArtikelen,
  ...
  const doImport = (e: any) => {
    ...
    excelImport(f, (d) => {
      ...
      if (Array.isArray(d.dropship_artikelen)) setDropshipArtikelen(d.dropship_artikelen);
