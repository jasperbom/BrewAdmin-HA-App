  ...
  render() {
    if (this.state.err) return (
      <div className="max-w-lg mx-auto mt-16 p-6 bg-red-50 rounded-xl border border-red-200">
        <div className="font-semibold text-red-700 mb-1">{t('err_boundary_titel')}</div>
        <div className="text-xs text-red-500 font-mono break-all mb-3">{this.state.err}</div>
        <button onClick={() => this.setState({err:null})} className="text-sm text-red-600 underline hover:text-red-800">{t('err_boundary_opnieuw')}</button>
      </div>
    )
...
function App() {
  ...
  // Merch die via de webshop verkocht wordt maar niet uit eigen voorraad
  // komt — zie utils/merch.ts.
  const [merchArtikelen, setMerchArtikelen] = useStore('merch_artikelen', []);
  const [merchVoorraadLog, setMerchVoorraadLog] = useStore('merch_voorraad_log', []);
  ...
  // Website-telemetrie (plugin Craftery Brouwerij): standaard helemaal uit.
  ...
  // Traceeroefeningen (handboek hoofdstuk 11) — ook append-only: het bewijs
  // dát er periodiek geoefend is, en met welke uitkomst.
  const [haccpTraceOefeningen, setHaccpTraceOefeningen] = useStore('haccp_trace_oefeningen', []);
  ...
  // Opschoning v4: de vier generieke CCP-definities uit de eerste HACCP-opzet
  // (kooktemperatuur, koelsnelheid, vergistingstemperatuur, pH wort) gaan uit.
  // De kritische beheerspunten van het handboek zijn CCP 1/2/3 (vrijgave,
  // sluitcontrole, etiketcontrole) en die vier waarden staan al in de
  // brouwdagstappen en de gistmetingen. Uitzetten, niet verwijderen: de
  // historie in haccp_ccp_metingen blijft en ze zijn in Instellingen →
  // Batchtaken weer aan te zetten. Pure logica + tests in src/utils/taken.ts.
  const [takenOpschoningV4, setTakenOpschoningV4] = useStore('batch_taken_opschoning_v4', null);
  const takenOpschoningV4Ref = React.useRef(false);
  React.useEffect(() => {
    if (takenOpschoningV4Ref.current) return;
    if (takenOpschoningV4 === 'v4') { takenOpschoningV4Ref.current = true; return; }
    if (takenMigratie !== 'v1' && takenMigratie !== 'done') return;
    const needed = ['batch_taken_items', 'batch_taken_opschoning_v4'];
    const ready = () => needed.every(k => _fetchedKeys.has(k));
    const run = () => {
      if (takenOpschoningV4Ref.current) return;
      takenOpschoningV4Ref.current = true;
      const res = deactiveerStandaardMetingen(Array.isArray(batchTakenItems) ? batchTakenItems : []);
      if (res.gewijzigd) setBatchTakenItems(res.items);
      setTakenOpschoningV4('v4');
    };
    if (ready()) { run(); return; }
    const int = setInterval(() => { if (ready()) { clearInterval(int); run(); } }, 500);
    const timeout = setTimeout(() => { clearInterval(int); run(); }, 8000); // fallback
  ...
  const doExport = () => {
    excelExport({
      ...
      artikelen,
      merch_artikelen: merchArtikelen, merch_voorraad_log: merchVoorraadLog,
      ...
      haccp_afwijkingen: haccpAfwijkingen,
      haccp_trace_oefeningen: haccpTraceOefeningen,
      haccp_instellingen: haccpInst,
      ...
      website_telemetrie: websiteTelemetrie,
  ...
  const doImport = (e: any) => {
    ...
    excelImport(f, (d) => {
      ...
      if (Array.isArray(d.merch_artikelen)) setMerchArtikelen(d.merch_artikelen);
      if (Array.isArray(d.merch_voorraad_log)) setMerchVoorraadLog(d.merch_voorraad_log);
      ...
      if (d.website_telemetrie && typeof d.website_telemetrie === 'object') setWebsiteTelemetrie(d.website_telemetrie);
  ...
  const resetApp = () => {
    ...
    // haccp_sluitcontroles, haccp_etiketcontroles, haccp_afwijkingen), de
    // traceeroefeningen (haccp_trace_oefeningen) en de afvulsessies staan:
    // bewaartermijn THT + 1 jaar, advies 5 jaar (HACCP-handboek §11.4). De
    // server weigert verwijdering ook hier.
  };
